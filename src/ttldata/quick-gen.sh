...
if [ "$actions" = "" ] ; then
  ...
  echo " "
  echo where actions is a space separated list ot these possible items: 
  echo \"cache ttl rdfhelp xml solr solr-publi solr-term solr-entries solr-gold-entries 
  echo gz runrq chr-reports hpp-reports peff ac-lists iso-md5 prepare-ftp archive-ftp 
  echo publish-ftp publish-ftp-skip-archive\"
  echo " "

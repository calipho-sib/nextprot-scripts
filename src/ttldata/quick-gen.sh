...
if [ "$actions" = "" ] ; then
  ...
  echo " "
  echo where actions is a space separated list ot these possible items: \"cache ttl rdfhelp xml solr solr-publi solr-term solr-entries solr-gold-entries 
  gz runrq chr-reports hpp-reports peff ac-lists iso-md5 prepare-ftp archive-ftp publish-ftp publish-ftp-skip-archive\"
  ...
  echo "- ttl:"
  echo "  builds the RDF turtle files, store them in directory export-ttl, load them in virtuoso, check files content consistency" 
  echo "- rdfhelp:"
  echo "  builds the RDF help based on SPARQL queries run by the API, depends on content generated by ttl action" 
  echo "- xml:" 
  echo "  builds the XML files, store them in directory export-xml, check files content consistency"
  echo "- solr:" 
  ...
  echo "  builds the solr index used for the API simple search system for entries taking into account only gold content" 
  echo "- gz:"
  echo "  touches the xml and ttl files with the given date and then compress them individually and store them in xml-compressed and ttl-compressed directories" 
  echo "- runrq:"
  ...
  echo "- prepare-ftp:"
  echo "  collects files generated by actions gz chr-reports hpp-reports ac-lists -iso-md5 peff iso-md5 plus NP1 controlled_vocabularies and mapping directories in a new directory prepared_ftp, note that a copy (except xml and ttl files) is also copied to M: for QC purpose" 
  echo "- archive-ftp:"
  echo "  archive the content of the current release directory of the ftp server in directory previous-releases, the name of the tar is based on the modification date of current_release directory" 
  echo "- publish-ftp:"
  echo "  performs the archive-ftp action and then publish the content of directory prepared-ftp to ftp server on current_release directory. All files on ftp server are touched with date chosen by user" 

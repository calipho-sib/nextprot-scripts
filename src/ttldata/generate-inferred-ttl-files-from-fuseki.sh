...
post_ok_cnt=$(grep -c "204 No Content" $fus_dir/fuseki-tdb.log)
get_ok_cnt=$(grep -c "200 OK" $fus_dir/fuseki-tdb.log)

status=OK
if [ "$post_cnt" != "6" ]; then status=ERROR; fi
if [ "$post_ok_cnt" != "6" ]; then status=ERROR; fi
if [ "$get_cnt" != "4" ]; then status=ERROR; fi
if [ "$get_ok_cnt" != "4" ]; then status=ERROR; fi
echo "Analyzing fuseki log file $fus_dir/fuseki-tdb.log : $status"
